...
const int DELTA_UP = -200; // cat de mult trebuie sa scada fata de centru pentru a fi "UP"
...
// ──────────────────── FUNCTII JOYSTICK ──────────────
bool joyUp(int pin, int center)      {
  return (analogRead(pin) - center) <  DELTA_UP; 
  }
bool joyReleased(int pin, int center){ 
  return (analogRead(pin) - center) > (DELTA_UP/2); 
  }
  ...
  for(int i = 0; i < 32; ++i) {
    if (i == 17 && !inJump) {
      ...
      lcd.write(byte(world[17])); 
        ...
        if (i == 16) 
          lcd.setCursor(0,1); lcd.write(byte(world[i])); 
        }
  }
  lcd.setCursor(13,0);
  lcd.print(score);
...
// ───────────── OBSTACOLE & COLIZIUNI (neschimbate) ─────────────
...
uint8_t scrollWorld() {
  delay(100);
  if (random(35) < 4) {
    if(random(10) < 4 && !hasObstacleInRow(0)) 
      world[15] = 4;               // pasare
    else if (!hasObstacleInRow(1))            
      world[31] = random(2,4);     // cactus
  } else world[15] = world[31] = 32;

  for(int i=0;i<16;++i)
    if(world[i]==4){
      if(i>0&&world[i-1]==0) return 1;
      world[i-1]=4; world[i] = (i < 15)?world[i+1]:32;
    }
  for (int i = 16 ;i < 32; ++i)
...
void setup() {
  ...
  lcd.clear(); 
  lcd.setCursor(0,1); 
  lcd.print("Swipe left-Start Game");
}

// ───────────────────── LOOP ─────────────────────────
void loop() {
  // START / RESTART cu joystick DREAPTA
  if (!joyUp(JOY_START_PIN,startCenter)) 
    return;
  while (!joyReleased(JOY_START_PIN,startCenter));
  ...
  while(true) {
    ...
    if (!canJump && joyReleased(JOY_JUMP_PIN,jumpCenter)) 
      canJump = true;
